...
void CompressRGBABlock_ExplicitAlpha(float rgbaBlock[4 * 4 * 4],
	...
	CompressExplicitAlphaBlock(alphaBlock, &compressedBlock[0]);
	CompressRGBBlock(rgbaBlock, &compressedBlock[2], pfChannelWeights, threeDRefinement, refinementSteps);
}

void CompressExplicitAlphaBlock(float const alphaBlock[4 * 4], uint32_t compressedBlock[2]) {
	...
	for (int i = 0; i < 16; i++) {
		int nBlock = i < 8 ? 0 : 1;
		uint8_t cAlpha = (uint8_t) (alphaBlock[i] * 255.0f);
		cAlpha = (uint8_t) (
...
void CompressAlphaBlock(float alphaBlock[4 * 4], uint32_t compressedBlock[2]) {
	...
	uint8_t nIndices[2][4 * 4];
	float fError8 = CompBlock1X(alphaBlock, 4 * 4, nEndpoints[0], nIndices[0], 8, false, 8, 0, true);
	float fError6 =
			(fError8 == 0.f) ? FLT_MAX : CompBlock1X(alphaBlock, 4 * 4, nEndpoints[1], nIndices[1], 6, true, 8, 0, true);
	if (fError8 <= fError6)
